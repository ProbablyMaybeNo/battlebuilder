...
import { OverheadBoard } from '../renderer/overhead';
      ...
      <main className="workspace" aria-label="Battle Builder workspace">
        <section className="canvas-stage" aria-label="Board workspace">
          ...
          <div className="canvas-void">
            ...
            {view === 'overhead' ? <OverheadBoard board={board} /> : <Suspense fallback={<p>Loading 3D planning view…</p>}><ThreeBoard board={board} preset={preset} onSelect={(id) => setSelectedPiece(board.pieces.find((piece) => piece.id === id) ?? null)} /></Suspense>} {view === '3d' && <div className="preset-bar"><Button onClick={() => setPreset('top')}>1 Top</Button><Button onClick={() => setPreset('isometric')}>2 Iso</Button><Button onClick={() => setPreset('perspective')}>3 Perspective</Button><Button onClick={() => setPreset('front')}>4 Front</Button></div>}<div className="zero-state"><div className="zero-state__glyph"><Crosshair size={32} /></div><p className="eyebrow">{board.pieces.length ? 'Starter layout staged' : '36 × 36 tactical surface'}</p><h1>{board.pieces.length ? 'Your terrain is ready for the board renderer.' : 'Start building the battlefield.'}</h1><p>{board.pieces.length ? `${board.pieces.length} terrain pieces are stored in the shared board document.` : 'Choose terrain from the catalog, or begin from a balanced starter layout.'}</p><div className="zero-state__actions"><Button variant="accent" onClick={() => { setTool('build'); setActiveDrawer('build'); }}>Build / add terrain</Button><Button variant="quiet" onClick={loadStarter}>Load starter layout</Button></div></div>
          </div>
        </section>
