...
export function App() {
  ...
  const select = (ids: string[]) => { setSelectedIds(ids); setMode('select'); announce(ids.length ? `${ids.length} ${ids.length === 1 ? 'terrain piece selected.' : 'terrain pieces selected.'}` : 'Selection cleared.'); };
  ...
  const enterBattle = () => {
    if (battleSession && battleSession.board.boardId === board.id && battleSession.board.snapshot.updatedAt === board.updatedAt) {
      setWorkspaceMode('battle'); setSelectedUnitId(null); setMode('select'); setActiveDrawer('roster'); announce('Returned to the preserved battle session.', 'success'); return;
    }
    ...
    setBattleSession(session); setSelectedUnitId(null); setWorkspaceMode('battle'); setMode('select'); setActiveDrawer('roster');
  ...
  const returnToBuild = () => { setWorkspaceMode('build'); setActiveDrawer(null); setSelectedUnitId(null); setMode('select'); announce('Returned to Build mode. Your board and battle session are both preserved.', 'success'); };
  ...
  const openBoard = (id: string) => {
    ...
    if (!result.ok) { announce(result.message, 'warning'); return; }
    command(result.value, `Opened ${result.value.name}.`, []); setOpenBoardsOpen(false); setMode('select');
  };
  const duplicateBoard = () => {
    const now = new Date().toISOString();
    const duplicate = parseBoard({ ...board, id: crypto.randomUUID(), name: `Copy of ${board.name}`, createdAt: now, updatedAt: now });
    command(duplicate, `${duplicate.name} created.`, []); setMode('select');
  };
  const renameBoard = () => {
  ...
  const importBoard = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      command(result.document, `Imported ${result.document.name}.`, []); setMode('select');
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if (primary && event.key.toLocaleLowerCase() === 'z') { event.preventDefault(); setHistory((value) => event.shiftKey ? redo(value) : undo(value)); setSelectedIds([]); setMode('select'); }
