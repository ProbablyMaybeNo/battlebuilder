...
export function ThreeBoard({ board, preset, selectedIds = [], onSelect }: { board: BoardDocument; preset: ThreePreset; selectedIds?: readonly string[]; onSelect?: (id: string | null) => void }) {
  const host = useRef<HTMLDivElement>(null); const canvasHost = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const element = canvasHost.current; if (!element) return undefined;
    ...
    const onKeyDown = (event: KeyboardEvent) => { const offset = camera.position.clone().sub(target); if (event.key.toLowerCase() === 'f') { setPreset(); event.preventDefault(); } else if (event.key === '+' || event.key === '=') { offset.multiplyScalar(.9); camera.position.copy(target).add(offset); event.preventDefault(); } else if (event.key === '-') { offset.multiplyScalar(1.1); camera.position.copy(target).add(offset); event.preventDefault(); } else { const move = event.key === 'ArrowLeft' ? [-1, 0] : event.key === 'ArrowRight' ? [1, 0] : event.key === 'ArrowUp' ? [0, -1] : event.key === 'ArrowDown' ? [0, 1] : null; if (!move) return; const amount = Math.max(.5, size / 40); camera.position.x += move[0] * amount; camera.position.z += move[1] * amount; target.x += move[0] * amount; target.z += move[1] * amount; event.preventDefault(); } camera.lookAt(target); };
    renderer.domElement.addEventListener('pointerdown', onPointerDown); renderer.domElement.addEventListener('pointermove', onPointerMove); renderer.domElement.addEventListener('pointerup', onPointerUp); renderer.domElement.addEventListener('pointercancel', finishPointer); renderer.domElement.addEventListener('lostpointercapture', finishPointer); renderer.domElement.addEventListener('wheel', onWheel, { passive: false }); renderer.domElement.addEventListener('contextmenu', (event) => event.preventDefault()); element.addEventListener('keydown', onKeyDown); window.addEventListener('blur', finishPointer);
    let frame = 0; const onContextLost = (event: Event) => { event.preventDefault(); if (host.current) host.current.dataset.contextLost = 'true'; }; const resize = () => { const { width, height } = element.getBoundingClientRect(); renderer.setSize(width, height); camera.aspect = width / height; camera.updateProjectionMatrix(); }; const render = () => { frame = requestAnimationFrame(render); renderer.render(scene, camera); }; renderer.domElement.addEventListener('webglcontextlost', onContextLost); resize(); const observer = new ResizeObserver(resize); observer.observe(element); render();
    return () => { cancelAnimationFrame(frame); observer.disconnect(); renderer.domElement.removeEventListener('webglcontextlost', onContextLost); renderer.domElement.removeEventListener('pointerdown', onPointerDown); renderer.domElement.removeEventListener('pointermove', onPointerMove); renderer.domElement.removeEventListener('pointerup', onPointerUp); renderer.domElement.removeEventListener('pointercancel', finishPointer); renderer.domElement.removeEventListener('lostpointercapture', finishPointer); renderer.domElement.removeEventListener('wheel', onWheel); element.removeEventListener('keydown', onKeyDown); window.removeEventListener('blur', finishPointer); renderer.dispose(); scene.traverse((object) => { if (object instanceof THREE.Mesh) { object.geometry.dispose(); const materials = Array.isArray(object.material) ? object.material : [object.material]; materials.forEach((material) => material.dispose()); } }); element.replaceChildren(); };
  }, [board, onSelect, preset, selectedIds]);
  return <div ref={host} className="three-board" role="application" aria-label="Interactive 3D planning board"><div ref={canvasHost} className="three-board__canvas" role="region" tabIndex={0} aria-label="3D camera: arrow keys pan, plus and minus zoom, F resets" /><ul className="sr-only" aria-label="3D terrain pieces">{board.pieces.filter((piece) => !piece.hidden).map((piece) => <li key={piece.id}><button type="button" aria-pressed={selectedIds.includes(piece.id)} onFocus={() => onSelect?.(piece.id)} onClick={() => onSelect?.(piece.id)}>{accessiblePieceName(piece)}</button></li>)}</ul></div>;
}
