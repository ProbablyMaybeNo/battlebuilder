...
import { layoutTerrainLabels, terrainState } from '../model/geometry';
...
export function OverheadBoard({ board, onCameraChange, selectedIds = [], hoveredId = null, onHoverPiece }: { board: BoardDocument; onCameraChange?: (camera: Camera) => void; selectedIds?: readonly string[]; hoveredId?: string | null; onHoverPiece?: (id: string | null) => void }) {
  ...
  return <div className="overhead-wrap"><svg ref={svgRef} className={`overhead-board surface-${board.settings.surface}`} viewBox={`${width / 2 - viewWidth / 2 + camera.x} ${height / 2 - viewHeight / 2 + camera.y} ${viewWidth} ${viewHeight}`} role="application" aria-label={`${width} by ${height} inch overhead board`} tabIndex={0} onWheel={(event) => { event.preventDefault(); update(zoomCamera(camera, event.deltaY < 0 ? 1.12 : 1 / 1.12)); }} onPointerDown={(event) => { if (event.button === 1 || event.shiftKey) { drag.current = { x: event.clientX, y: event.clientY }; event.currentTarget.setPointerCapture(event.pointerId); } }} onPointerMove={(event) => { const rect = event.currentTarget.getBoundingClientRect(); setCursor(screenToBoard(event.clientX, event.clientY, rect, camera, board)); if (drag.current) { update(panCamera(camera, event.clientX - drag.current.x, event.clientY - drag.current.y)); drag.current = { x: event.clientX, y: event.clientY }; } }} onPointerUp={() => { drag.current = null; }} onPointerLeave={() => onHoverPiece?.(null)}>
    ...
    {board.pieces.slice().sort((left, right) => left.layer - right.layer).map((piece) => <g key={piece.id} onPointerEnter={(event) => { event.stopPropagation(); onHoverPiece?.(piece.id); }}><TerrainPieceSvg piece={piece} state={terrainState(piece, selectedIds, hoveredId)} label={labels.get(piece.id)} /></g>)}
