...
for (const viewport of [{ width: 1440, height: 960 }, { width: 1920, height: 1080 }]) {
  test(`captures accessible focused board at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    await piece.focus();
    await page.screenshot({ path: `ux-audit/b09-accessibility-${viewport.width}x${viewport.height}.png`, fullPage: true });
  });
}
