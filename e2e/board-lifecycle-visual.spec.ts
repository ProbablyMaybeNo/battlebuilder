...
for (const viewport of [{ width: 1440, height: 960 }, { width: 1920, height: 1080 }]) {
  test(`captures lifecycle import recovery at ${viewport.width}x${viewport.height}`, async ({ page }) => {
    ...
    await page.screenshot({ path: `ux-audit/b08-import-recovery-${viewport.width}x${viewport.height}.png`, fullPage: true });
  });
}
